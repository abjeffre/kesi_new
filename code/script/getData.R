...
#   
  ...
  h$hours[1] = d$ag$weed_hours[1] + d$ag$weed_hours[1] + d$ag$harvest_hours[1]
...
#   
  ...
  for(i in 1:nrow(d$ag)){
    di <- h2
    di$hhID = d$ag$hhID[i]
    di$hours = d$ag$plant_hours[i] + d$ag$[i] + d$ag$harvest_hours[i]
  ...
  # Clove Hours
    ...
    di <- h2
    di$hhID = d$ag$hhID[i]
    di$hours =  d$ag$prepare_cloves_hours[i] + d$ag$harvest_cloves_hours[i]
    di$name = d$ag$[i]
    di$period = d$ag$period[i]
    di$shehia = d$ag$shehia[i]
    di$julian = d$ag$julian[i]
...
# Mwani Hours
  ...
  for(i in 1:nrow(db2$mwani_emp_source)){
    ...
    di$hours =  db2$mwani_emp_source$harvest_mwani_hours[i]+db2$mwani_emp_source$prepare_mwani_hours[i]
    di$name =   db2$mwani_emp_source$names[i]
  ...
  for(i in 1:nrow(d$animal_emp)){
    ...
    di$hours = d$animal_emp$hours[i]
    di$name = d$animal_emp$hours[i]
...
# ADD IN MEDICINE 
  ...
  for(i in 1:nrow(d$forest)){
    ...
    di$hours = d$forest$hours[i]
    di$name = d$forest$names[i]
  ...
  for(i in 1:nrow(d$rent)){
    ...
    di$type = d$rent$other[i]
    di$main = "rent"
    di$value = d$rent$price[i]
  ...
  for(i in 1:nrow(d$sold)){
    ...
    di$type = d$sold$other[i]
    di$main = "sold"
    di$value = d$sold$price[i]
  ...
  for(i in 1:nrow(d$store_credit)){
    ...
    di$type = d$store_credit$items[i]
    di$main = "store_credit"
    di$value = d$store_credit$value[i]
  ...
  for(i in 1:nrow(d$gift_rec)){
    types <-str_split(d$gift_rec$type[i], " ")[[1]]
    for(j in 1:length(types)){
      di <- v2
      di$hhID = d$gift_rec$hhID[i]
      di$type = ifelse(types[j] == "nyingine", d$gift_rec$type_other[i], types[j])
      di$main = "gift_rec"
      di$value = d$gift_rec$value[i]/length(types)
      di$period = d$gift_rec$period[i]
      di$location = ifelse(d$gift_rec$location[i] == "nyingine", d$gift_rec$location_other[i], d$gift_rec$location[i])  
      di$subtype = ifelse(d$gift_rec$source[i] == "nyingine", d$gift_rec$source_other[i], d$gift_rec$source[i])
  ...
  for(i in 1:nrow(d$gift_give)){
    types <-str_split(d$gift_give$type[i], " ")[[1]]
    for(j in 1:length(types)){
      di <- v2
      di$hhID = d$gift_give$hhID[i]
      di$type = ifelse(types[j] == "nyingine", d$gift_give$type_other[i], types[j])
      di$main = "gift_give"
      di$value = d$gift_give$value[i]/length(types)
      di$location = ifelse(d$gift_give$location[i] == "nyingine", d$gift_give$location_other[i], d$gift_give$location[i])  
      di$period = d$gift_give$period[i]
      di$subtype = ifelse(d$gift_give$source[i] == "nyingine", d$gift_give$source_other[i], d$gift_give$source[i])  
