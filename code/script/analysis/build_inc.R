...
v$date <- d$self$"_submission_submission_time"
v$note <- d$self$notes
...
v$date <- d$wage$"_submission_submission_time"
v$note <- d$wage$notes
...
# Seaweed
ind = which(!is.na(d$main$mwani_sold_amount)) 
v <- init(length(ind))
v$income=d$main$mwani_sold_amount[ind]*ifelse(is.na(d$main$mwani_price[ind]), 1100, d$main$mwani_price[ind])
v$type = "mwani"
...
v$type = "ag_main"
v$subtype = d$ag_crop$type[ind]
v$date = d$ag_crop$`_submission_submission_time`[ind]
...
v$date = d$forest_crop$`_submission_submission_time`[ind]
v$interviewer = d$forest_crop$`_submission_submitted_by`[ind]
...
for(i in ind){
  ...
  if((d$forest_crop$harvest[i] >0 & !is.na(d$forest_crop$sold[i])) & !is.na(d$forest_crop$harvest[i])){
    ...
    di$interviewer = d$forest_crop$`_submission_submitted_by`[i]
    dp <- rbind(dp, di)
  }
}
...
v$interviewer <- d$safari$`_submission_submitted_by`
v$shehia <- d$safari$shehia
v$date <- d$safari$"_submission_submission_time"
...
v$interviewer <- d$sold$`_submission_submitted_by`
v$shehia <- d$sold$shehia
v$date <- d$sold$"_submission_submission_time"
v$note <- d$sold$notes
...
v$interviewer <- d$rent$`_submission_submitted_by`
v$shehia <- d$rent$shehia
v$date <- d$rent$"_submission_submission_time"
v$note <- d$rent$notes
...
#######################################################
#################### OUTLIER REMOVAL ##################
dp[-which(dp$type == "livestock" & dp$income == 1700000),]
dp[-which(dp$type == "ntfp" & dp$income == 1050000),]
...
sec[grepl("uvuvi", dp$type)] <- "fishing"
...
sec[grepl("diwani", dp$type)] <- "government"
...
sec[grepl("hotel", dp$type)] <- "other services" 
sec[grepl("kaya maskini", dp$type)] <- "other services" 
...
##################################################
######## JANUARY 2024 UPDATE ##########################
sec[grepl("chw's", dp$type)] <- "other services" 
sec[grepl("kushona nyavu", dp$type)] <- "fishing" 
sec[grepl("manual labor", dp$type)] <- "manufacturing" 
sec[grepl("kibarua ya bandari", dp$type)] <- "other services" 
sec[grepl("radio", dp$type)] <- "other services" 
sec[grepl("kuuza vifuko", dp$type)] <- "retail"
sec[grepl("kibarua ya seremala", dp$type)] <- "manufacturing"
sec[grepl("kutengeneza kamba", dp$type)] <- "manufacturing"
sec[grepl("pottery", dp$type)] <- "manufacturing"
sec[grepl("kuuza mitumba nyumbani", dp$type)] <- "retail"
sec[grepl("accountant", dp$type)] <- "government"
sec[grepl("temporary gov", dp$type)] <- "government"
sec[grepl("jku", dp$type)] <- "government"
sec[grepl("kuuza miche", dp$type)] <- "agroforestry" 
sec[grepl("kutengeneza mafuta", dp$type)] <- "food services" 
sec[grepl("kuokota chupa", dp$type)] <- "other services" 
sec[grepl("kibarua kugasiha", dp$type)] <- "biashara ya kilimo" 
sec[grepl("kuchora hena", dp$type)] <- "other services" 
sec[grepl("kibarua ya skuli", dp$type)] <- "other services" 
sec[grepl("mkaa", dp$type)] <- "timber" 


############################################
########## CLEANING SALES ##################

sort(table(dp$subtype[which(dp$type=="sale")]))

sec[which(dp$type == "sale" & dp$subtype == "mwani")] <- "mwani"
sec[which(dp$type == "sale" & dp$subtype == "karafu")] <- "cloves"
sec[which(dp$type == "sale" & dp$subtype == "coconut")] <- "agriculture"
sec[which(dp$type == "sale" & dp$subtype == "charcoal")] <- "timber"
sec[which(dp$type == "sale" & dp$subtype == "clothing")] <- "retail"
sec[which(dp$type == "sale" & dp$subtype == "kuukuu")] <- "livestock"
sec[which(dp$type == "sale" & dp$subtype == "fish")] <- "fishing"
sec[which(dp$type == "sale" & dp$subtype == "milk")] <- "livestock"
sec[which(dp$type == "sale" & dp$subtype == "banana")] <- "agriculture"
sec[which(dp$type == "sale" & dp$subtype == "cassava")] <- "agriculture"
sec[which(dp$type == "sale" & dp$subtype == "peanuts")] <- "agriculture"
sec[which(dp$type == "sale" & dp$subtype == "cow")] <- "livestock"
sec[which(dp$type == "sale" & dp$subtype == "kuukuu")] <- "livestock"
sec[which(dp$type == "sale" & dp$subtype == "goat")] <- "livestock"
sec[which(dp$type == "sale" & dp$subtype == "mabalungi passion avocado")] <- "agriculture"
sec[which(dp$type == "sale" & dp$subtype == "milk banana")] <- "agriculture"
sec[which(dp$type == "sale" & dp$subtype == "milk fish")] <- "agriculture"
...
############# NEW CORRECTIONS JAN 30 2024 ##############
# Get list of sec that fails (right now entires less than 23)
# We want to the fastest way is to simply replace them here and find a way to import back into the data-set later
...
for(t in 2:(max(dp$period)-1)){
  print(t)
  for(k in unique(dp$sector)){
    ind = which((dp$period ==t & dp$sector == k) & dp$shehia %in% use_shehia)
    period = c(period, t)
    sector = c(sector, k)
    n = c(n, length(unique(d$main$hh_ID[dp$shehia %in% use_shehia & dp$period ==t])))
    gsp = c(gsp, sum(dp$income[ind],na.rm =T))
