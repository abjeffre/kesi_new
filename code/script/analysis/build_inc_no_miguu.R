...
sec[grepl("uvuvi", dp$type) & !grepl("miguu", dp$subtype)] <- "fishing" # Note this removes all female harvesting
...
for(t in 2:(max(dp$period)-1)){
  print(t)
  for(k in unique(dp$sector)){
    ind = which((dp$period ==t & dp$sector == k) & dp$shehia %in% c("msuka_magharibi", "kifundi"))
    period = c(period, t)
    sector = c(sector, k)
    gsp = c(gsp, sum(dp$income[ind],na.rm =T))
