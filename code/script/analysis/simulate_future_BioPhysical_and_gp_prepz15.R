...
#wind
wind_breaks <-seq( min(wind_pro_high)-.25, max(wind_pro_high)+.3, length.out = 17)
...
#waves
wave_breaks <- seq( min(wave_pro_high)-.2, max(wave_pro_high)+.4, length.out = 17)
...
# moisture
moisture_breaks <- seq( min(moisture_pro_high)-.15, max(moisture_pro_high)+1.1, length.out = 17)
...
#Zscore
zscore_breaks <- seq( min(zscore_pro_high)-.3, max(zscore_pro_high)+.3, length.out = 17)
...
#heat
heat_breaks <- seq( min(heat_pro_high)-.15, max(heat_pro_high)+.4, length.out = 17)
...
# Note that this is already in Zscore
zscore_pro_bau = rep(0, max(period))
...
moisture_pro_drought<-ifelse(moisture_pro_bau+1 < 0, 0, moisture_pro_bau+1)
zscore_pro_drought <- rep(1, 26)
heat_pro_drought <- heat_pro_bau +1.5
wind_pro_drought <- wind_pro_bau
wave_pro_drought <- wave_pro_bau

wind_cat_pro_drought<- cut(wind_pro_drought, breaks = wind_breaks, right = FALSE)
wave_cat_pro_drought<- cut(wave_pro_drought, breaks = wave_breaks, right = FALSE)
zscore_cat_pro_drought<- cut(zscore_pro_drought, breaks = zscore_breaks, right = FALSE)
moisture_cat_pro_drought<- cut(moisture_pro_drought, breaks = moisture_breaks, right = FALSE)
heat_cat_pro_drought<- cut(heat_pro_drought, breaks = heat_breaks, right = FALSE)


df_drought <- data.frame(heat = heat_pro_drought[imp_month],
                         heat_cat = heat_cat_pro_drought[imp_month],
                         wind = wind_pro_drought[imp_month],
                         wind_cat = wind_cat_pro_drought[imp_month],
                         wave = wave_pro_drought[imp_month],
                         wave_cat = wave_cat_pro_drought[imp_month],
                         moisture = moisture_pro_drought[imp_month],
                         moisture_cat = moisture_cat_pro_drought[imp_month],
                         zscore = zscore_pro_drought[imp_month],
                         zscore_cat = zscore_cat_pro_drought[imp_month])
